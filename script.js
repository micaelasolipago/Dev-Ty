...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (window.matchMedia('(hover: hover)').matches) {
        document.querySelectorAll('.craft-item').forEach(card => {
            card.addEventListener('mouseenter', () => {
                ...
                if (number) number.style.opacity = '0.6';
                if (visual) visual.style.opacity = '0.6';
                if (visual) visual.style.transform = 'scale(1.1)';
            });
            ...
            card.addEventListener('mouseleave', () => {
                ...
                if (number) number.style.opacity = '0.3';
    ...
    if (window.matchMedia('(hover: hover)').matches) {
        document.querySelectorAll('.work-item').forEach(item => {
            item.addEventListener('mouseenter', () => {
                ...
                if (preview) preview.style.transform = 'scale(1.02)';
    ...
    function handleResize() {
        ...
        if (isMobile && isMenuOpen && navMenu) {
            navMenu.style.display = 'none';
            isMenuOpen = false;
...
// Fallback for theme toggle if DOMContentLoaded doesn't fire
window.addEventListener('load', function() {
    const themeToggle = document.getElementById('theme-toggle');
    const themeIcon = document.querySelector('.theme-icon');
    const html = document.documentElement;
    
    if (themeToggle && themeIcon && !html.getAttribute('data-theme')) {
